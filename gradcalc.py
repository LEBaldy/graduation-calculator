...
Var, FinalOutput, Ft, GradSection, UpperLimits, sigma_skips= [CurrentStudents, Tau, Phi, Phi + Tau, CurrentStudents * 200 + 1000], "N/A", "N/A", "N/A", "N/A", [166, 190, 216, 220, 240, 246, 251, 256, 272, 289, 298, 307, 310, 316, 324, 326, 334]
...
Equations_of_Doom, Other_Data, Error_Collection_Grad = sheet.get_worksheet(7), sheet.get_worksheet(8), sheet.get_worksheet(9)
...
try:
  ...
  def GradCalc(Tau, Phi, CurrentStudents):
      ...
      if (Var[1] > 0):  #No Theories Check
        if (Var[0] >= 20):  #Tau under 5k Check
          if (Var[3] <= int(Other_Data.cell(col=11, row=2).value)):Section = 55  #Section Sorting
            ...
            if (Var[0] >= 65):
              if (Var[0] >= 233 and Var[3] >= int(Other_Data.cell(col=12, row=2).value)): Section = 151
    ...
    try:
      ...
      if (Var[1] < 0 or Var[2] < 0 or Var[0] < 5):FinalOutput = str("Please Input Valid Values for Phi, Tau, and Total Students")  #Valid Input No Calculation Check
        ...
        elif(2000>Ft[0] or Ft[0]>60000): print(bounds)
      ...
      print("\n")
  Calcput=GradCalc(Tau, Phi, CurrentStudents)
  ...
  pass
  Error_Collection_Grad.append_row([str(err), Var[0], Var[1], Var[2], FinalOutput, Ft, GradSection, UpperLimits])
  print("An error occured.\nBug report has been sent for inspection. Status can be found here: https://docs.google.com/spreadsheets/d/1lz_V0rCr3usm1Wyqn2DEJdw6_7lhWIxXCYFmUbArV1c/edit#gid=1086488695")